...
class OpenPoseDetectorWrapper:
    ...
    def __call__(self, input_image, detect_resolution=512, **kwargs):
        # Handle both torch.Tensor and numpy.ndarray inputs
        if isinstance(input_image, torch.Tensor):
            input_image_np = (input_image.cpu().numpy() * 255).astype(np.uint8)
        ...
        processed_image_pil = self.model(input_image_pil, detect_resolution=detect_resolution, image_resolution=detect_resolution)
        processed_image_np = np.array(processed_image_pil)
        if processed_image_np.ndim == 2:
            processed_image_np = processed_image_np[:, :, None]
        if processed_image_np.ndim == 3:
            H, W, C = processed_image_np.shape
            if C == 1 or C == 3 or C == 4:
                return processed_image_np
            else:
                if C == 1:
                    processed_image_np = np.concatenate([processed_image_np, processed_image_np, processed_image_np], axis=2)
                elif C > 4:
                    processed_image_np = processed_image_np[:, :, :3]
                return processed_image_np
        return processed_image_np
...
class OpenPose_Preprocessor:
    def execute(self, image, resolution=512, **kwargs):
        model_wrapper = OpenPoseDetectorWrapper.from_pretrained()
        out = common_annotator_call(model_wrapper, image, resolution=resolution)
        return (out, )
...
class MiDaSDetector:
    ...
    def __call__(self, input_image, detect_resolution=512, output_type="pil", upscale_method="INTER_CUBIC", **kwargs):
        ...
        # Ensure depth has the correct number of dimensions and channels
        if depth.ndim == 2:
            depth = depth[:, :, None]  # Add a channel dimension
        if depth.ndim == 3:
            H, W, C = depth.shape
            if C == 1 or C == 3 or C == 4:
